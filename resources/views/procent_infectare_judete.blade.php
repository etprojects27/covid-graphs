    ...
    @foreach($date_covid_judete_7_zile as $date_judet)
            <div class="col-lg-3 col-lm-2 col-ls-1 include-content">
                ...
                <script>
                    ...
                    cazuri_noi_string = '<?php echo $date_judet->cazuri_totale_covid_7_zile ?>';
                    populatie_judet = '<?php echo $date_judet->populatie ?>';
                    cazuri_totale = cazuri_noi_string.split(',');
                    
                    data_array = [];
                    for (i =0; i<cazuri_totale.length; i++){
                        procent_infectare_judet = cazuri_totale[i] / populatie_judet * 100;
                        data_array.push(procent_infectare_judet.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,'));
                    }                   
